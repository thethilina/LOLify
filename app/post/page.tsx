  ...
  const handlepost = async () => {
    try {
      ...
      const memeres = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/memes/protected`,
        {
          method: "POST",
          mode: "no-cors",
          headers: {
  ...
  return (
    <div className=" items-start h-full flex flex-col  justify-center   mx-50 my-30 gap-y-10  text-lg text-[#B9B9CE] ">
      ...
      <input
        ...
        type="text"
        placeholder="Title"
        className="border border-gray-500  rounded-xl  w-150   py-2 px-4  bg-[#1F1F24] focus:border-[#878b87] focus:outline-none "
      />
      ...
      <button
        ...
        className=" bg-[#303030]   px-3 hover:cursor-pointer hover:bg-[#444444] py-1 rounded-2xl "
      >
        Add Tags
      </button>
      ...
      {meme ? (
        <div className="w-100 h-100  bg-[#0c0c0c] flex border-dotted items-start  justify-end rounded-xl ">
          <MdOutlineDelete
            size={30}
            className="hover:cursor-pointer  p-1 m-2 bg-gray-900 rounded-full  text-gray-300 hover:bg-gray-800 z-50"
            onClick={() => {
              setMeme(null);
            }}
          />
          <Image
            ...
            width={150}
            height={150}
            className="h-100 w-100 absolute object-contain rounded-xl border border-gray-500 border-dotted"
            alt="dfsg"
          />
        </div>
      ) : (
        <div
          id="upload"
          className="w-150 h-50 gap-x-5  bg-[#1F1F24] flex border-dotted items-center justify-center rounded-xl border border-gray-500 "
        >
          <input
            onChange={handleProfileChange}
            type="file"
            accept=".jpg,.jpeg,.png"
            className="absolute w-150 h-50 opacity-0 cursor-pointer"
          />
          <h1>Attach Your Meme</h1>
          <LuUpload size={25} />
        </div>
      )}
      ...
      <button
        onClick={handlepost}
        className="font-medium bg-[#4b4679]    px-7 hover:cursor-pointer hover:bg-[#6d62cf] py-1 rounded-xl "
      >
        Post
      </button>
        ...
        >
          <div className="flex flex-col  items-center justify-center rounded-xl border border-gray-400  bg-[#0F0F11] w-70 p-5 b gap-y-5 h-50">
            <input
              ...
              type="text"
              placeholder="tagline"
              className="border border-gray-500  rounded-xl     py-1 px-2  bg-[#1F1F24] focus:border-[#878b87] focus:outline-none"
            />
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="px-2 py-1 bg-gray-600 rounded-xl hover:cursor-pointer"
            >
              Add
            </button>
          </div>
